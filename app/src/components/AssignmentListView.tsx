  ...
  return (
    <div className="space-y-4">
      {assignments.length === 0 ? (
        <div className="bg-white/85 rounded-xl border border-neutral-200 p-12 text-center">
          <p className="text-neutral-500">Keine Aufträge verfügbar</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {assignments.map((assignment) => {
            const startDateTime = formatDateTime(assignment.start);
            const endDateTime = formatDateTime(assignment.end);
            ...
            return (
              <motion.div
                ...
                className="bg-white/85 rounded-xl border border-neutral-200 hover:border-primary-300 transition-all overflow-hidden"
              >
                <div className="p-6">
                  {/* Header */}
                  ...
                  {assignment.status === 'OPEN' && matchScore > 0 && (
                    <div className="mb-4 p-3 bg-linear-to-r from-neutral-50 to-primary-50 rounded-lg border border-neutral-200">
                          ...
                          <div className="w-32 h-2 bg-white rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={`h-full ${
                                ...
                                matchScore >= 60 ? 'bg-secondary-500' :
                                'bg-warning'
                              }`}
                            />
                          </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-neutral-700">PLZ {assignment.address.zipCode}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                  ...
                  {assignment.status === 'OPEN' && onAccept && onReject && (
                    <div className="flex gap-2 pt-4 border-t border-neutral-200">
                      <button
                        ...
                        className="flex-1 px-4 py-2.5 btn-base btn-ghost font-medium text-sm flex items-center justify-center gap-2"
                      >
                        <X className="w-4 h-4" />
                        Ablehnen
                      </button>
                      <button
                        ...
                        className="btn-base btn-secondary flex-1 px-4 py-2.5 text-sm flex items-center justify-center gap-2"
                      >
